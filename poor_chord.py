...
class Node:
    ...
    def stabilize(self):
        ...
        self.client_requester.destination_id, self.client_requester.destination_addr = self.predeccesor_id, self.predeccesor_addr
...
#
    ...
    def update_others(self , introduction_node = False):
        ...
        for i in range( 1, self.m + 1):
            ...
            recieved_json = self.client_requester.make_request(json_to_send = {"command": "UPD_FING", "params": { "new_node_addr" : self.addr, "new_node_id" : self.id, "index_to_actualize" : i -1, "addr_requester": self.addr } })
...
#
    ...
    def find_predecesor(self, id):
        ...
        while not self.belongs_to(id, interval = (current_id, current_succ_id)) and current_id != current_succ_id :
            ...
            self.client_requester.destination_id, self.client_requester.destination_addr = (current_id, current_succ_id)
            recv_json_closest = self.client_requester.make_request(json_to_send = {"command_name" : "CLOSEST_PRED_FING", "method_params" : {"id": id}, "procedence_addr" : self.addr}, method_for_wrap = 'closest_pred_fing_wrap', requester_object= self)                        
            ...
            self.client_requester.destination_id, self.client_requester.destination_addr = (recv_json_closest['return_info']['closest_id'], recv_json_closest['return_info']['closest_addr'])
            recv_json_succ = self.client_requester.make_request(json_to_send = {"command_name" : "GET_SUCC_LIST", "method_params" : {}, "procedence_addr" : self.addr}, requester_object = self, asked_properties = ("succ_list", ) )            
